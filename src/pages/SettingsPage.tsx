...
import {
  SideNavLayoutMode,
  useSideNavLayoutStore,
} from "../stores/useSideNavLayoutStore";
...
export function SettingsPage() {
  ...
  const [activeTab, setActiveTab] = useState<"general" | "network" | "about">(
    "general",
  );
  const orderedPlatformIds = usePlatformLayoutStore(
    (state) => state.orderedPlatformIds,
  );
  const platformSettingsOrder = useMemo<Record<PlatformId, number>>(() => {
    const next: Record<PlatformId, number> = {
      ...FALLBACK_PLATFORM_SETTINGS_ORDER,
    };
    let order = 0;
  ...
  const [minimizeBehavior, setMinimizeBehavior] = useState<
    "dock_and_tray" | "tray_only"
  >("dock_and_tray");
  const [hideDockIcon, setHideDockIcon] = useState(false);
  const [floatingCardShowOnStartup, setFloatingCardShowOnStartup] =
    useState(true);
  ...
  const [codebuddyQuotaAlertEnabled, setCodebuddyQuotaAlertEnabled] =
    useState(false);
  const [codebuddyQuotaAlertThreshold, setCodebuddyQuotaAlertThreshold] =
    useState("20");
  const [codebuddyCnQuotaAlertEnabled, setCodebuddyCnQuotaAlertEnabled] =
    useState(false);
  const [codebuddyCnQuotaAlertThreshold, setCodebuddyCnQuotaAlertThreshold] =
    useState("20");
  const [qoderQuotaAlertEnabled, setQoderQuotaAlertEnabled] = useState(false);
  const [qoderQuotaAlertThreshold, setQoderQuotaAlertThreshold] =
    useState("20");
  ...
  const [workbuddyQuotaAlertEnabled, setWorkbuddyQuotaAlertEnabled] =
    useState(false);
  const [workbuddyQuotaAlertThreshold, setWorkbuddyQuotaAlertThreshold] =
    useState("20");
  const [codebuddyAutoRefreshCustomMode, setCodebuddyAutoRefreshCustomMode] =
    useState(false);
  const [
    codebuddyCnAutoRefreshCustomMode,
    setCodebuddyCnAutoRefreshCustomMode,
  ] = useState(false);
  const [workbuddyAutoRefreshCustomMode, setWorkbuddyAutoRefreshCustomMode] =
    useState(false);
  const [
    codebuddyQuotaAlertThresholdCustomMode,
    setCodebuddyQuotaAlertThresholdCustomMode,
  ] = useState(false);
  const [qoderAutoRefreshCustomMode, setQoderAutoRefreshCustomMode] =
    useState(false);
  const [
    qoderQuotaAlertThresholdCustomMode,
    setQoderQuotaAlertThresholdCustomMode,
  ] = useState(false);
  const [traeAutoRefreshCustomMode, setTraeAutoRefreshCustomMode] =
    useState(false);
  const [
    traeQuotaAlertThresholdCustomMode,
    setTraeQuotaAlertThresholdCustomMode,
  ] = useState(false);
  const [zedAutoRefreshCustomMode, setZedAutoRefreshCustomMode] =
    useState(false);
  const [
  ...
  const [appPathResetDetectingTargets, setAppPathResetDetectingTargets] =
    useState<Set<AppPathTarget>>(new Set());
  const [opencodeSyncOnSwitch, setOpencodeSyncOnSwitch] = useState(false);
  const [opencodeAuthOverwriteOnSwitch, setOpencodeAuthOverwriteOnSwitch] =
    useState(false);
  ...
  const [autoSwitchThreshold, setAutoSwitchThreshold] = useState("0");
  ...
  const [codexQuotaAlertThreshold, setCodexQuotaAlertThreshold] =
    useState("20");
  ...
  const [windsurfQuotaAlertEnabled, setWindsurfQuotaAlertEnabled] =
    useState(false);
  const [windsurfQuotaAlertThreshold, setWindsurfQuotaAlertThreshold] =
    useState("20");
  ...
  const [cursorQuotaAlertThreshold, setCursorQuotaAlertThreshold] =
    useState("20");
  const [geminiQuotaAlertEnabled, setGeminiQuotaAlertEnabled] = useState(false);
  const [geminiQuotaAlertThreshold, setGeminiQuotaAlertThreshold] =
    useState("20");
  ...
  const [windsurfAutoRefreshCustomMode, setWindsurfAutoRefreshCustomMode] =
    useState(false);
  const [kiroAutoRefreshCustomMode, setKiroAutoRefreshCustomMode] =
    useState(false);
  const [cursorAutoRefreshCustomMode, setCursorAutoRefreshCustomMode] =
    useState(false);
  const [geminiAutoRefreshCustomMode, setGeminiAutoRefreshCustomMode] =
    useState(false);
  const [autoSwitchThresholdCustomMode, setAutoSwitchThresholdCustomMode] =
    useState(false);
  const [quotaAlertThresholdCustomMode, setQuotaAlertThresholdCustomMode] =
    useState(false);
  const [
  ...
  const [
    ...
    setGeminiQuotaAlertThresholdCustomMode,
  ] = useState(false);
  const [
  ...
  useEffect(() => {
    const handleStarted = (event: Event) => {
      const detail = (event as CustomEvent<{ source?: UpdateCheckSource }>)
        .detail;
  ...
  const reportPreviewPort =
    reportActualPort ?? (parseInt(reportPort, 10) || reportDefaultPort);
  const reportPreviewToken = encodeURIComponent(
    (reportToken || "your-token").trim() || "your-token",
  );
  ...
  useEffect(() => {
    ...
    const codebuddyCnAutoRefreshNum =
      parseInt(codebuddyCnAutoRefresh, 10) || -1;
    ...
    const parsedCodexQuotaAlertThreshold = Number.parseInt(
      codexQuotaAlertThreshold,
      10,
    );
    const parsedGhcpQuotaAlertThreshold = Number.parseInt(
      ghcpQuotaAlertThreshold,
      10,
    );
    const parsedWindsurfQuotaAlertThreshold = Number.parseInt(
      windsurfQuotaAlertThreshold,
      10,
    );
    const parsedKiroQuotaAlertThreshold = Number.parseInt(
      kiroQuotaAlertThreshold,
      10,
    );
    const parsedCodebuddyQuotaAlertThreshold = Number.parseInt(
      codebuddyQuotaAlertThreshold,
      10,
    );
    const parsedCodebuddyCnQuotaAlertThreshold = Number.parseInt(
      codebuddyCnQuotaAlertThreshold,
      10,
    );
    const parsedWorkbuddyQuotaAlertThreshold = Number.parseInt(
      workbuddyQuotaAlertThreshold,
      10,
    );
    const parsedQoderQuotaAlertThreshold = Number.parseInt(
      qoderQuotaAlertThreshold,
      10,
    );
    const parsedTraeQuotaAlertThreshold = Number.parseInt(
      traeQuotaAlertThreshold,
      10,
    );
    const parsedZedQuotaAlertThreshold = Number.parseInt(
      zedQuotaAlertThreshold,
      10,
    );
    const parsedCursorQuotaAlertThreshold = Number.parseInt(
      cursorQuotaAlertThreshold,
      10,
    );
    const parsedGeminiQuotaAlertThreshold = Number.parseInt(
    ...
    generalSaveTimerRef.current = window.setTimeout(async () => {
      try {
        await invoke("save_general_config", {
          ...
          quotaAlertEnabled,
          quotaAlertThreshold: Number.isNaN(parsedQuotaAlertThreshold)
            ? 20
          ...
          windsurfQuotaAlertThreshold: Number.isNaN(
          ...
          codebuddyQuotaAlertThreshold: Number.isNaN(
            parsedCodebuddyQuotaAlertThreshold,
          )
            ? 20
            : parsedCodebuddyQuotaAlertThreshold,
          codebuddyCnQuotaAlertEnabled,
          codebuddyCnQuotaAlertThreshold: Number.isNaN(
            parsedCodebuddyCnQuotaAlertThreshold,
          )
            ? 20
            : parsedCodebuddyCnQuotaAlertThreshold,
          workbuddyQuotaAlertEnabled,
          workbuddyQuotaAlertThreshold: Number.isNaN(
          ...
          cursorQuotaAlertThreshold: Number.isNaN(
            parsedCursorQuotaAlertThreshold,
          )
            ? 20
            : parsedCursorQuotaAlertThreshold,
          geminiQuotaAlertEnabled,
          geminiQuotaAlertThreshold: Number.isNaN(
        ...
        alert(
          `${t("settings.network.saveFailed").replace("{error}", String(err))}`,
        );
      }
    }, 300);
  ...
  useEffect(() => {
    const checkResetTasks = () => {
      try {
        ...
        const hasReset =
          Array.isArray(tasks) &&
          tasks.some((task: any) => task.enabled && task.schedule?.wakeOnReset);
    ...
    const handleStorageChange = (e: StorageEvent) => {
      if (
        e.key === "agtools.wakeup.tasks" ||
        e.key === "agtools.wakeup.enabled"
      ) {
        checkResetTasks();
      }
    };
  ...
  const applyUiScale = async (rawScale: string) => {
    ...
    const normalized = Number.isFinite(parsed)
      ? Math.min(2, Math.max(0.8, parsed))
      : 1;
    try {
  ...
  const loadGeneralConfig = async () => {
    try {
      ...
      setFloatingCardShowOnStartup(
        config.floating_card_show_on_startup ?? true,
      );
      ...
      setCodebuddyAutoRefresh(
        String(config.codebuddy_auto_refresh_minutes ?? 10),
      );
      setCodebuddyCnAutoRefresh(
        String(config.codebuddy_cn_auto_refresh_minutes ?? 10),
      );
      setWorkbuddyAutoRefresh(
        String(config.workbuddy_auto_refresh_minutes ?? 10),
      );
      ...
      setCodebuddyQuotaAlertEnabled(
        config.codebuddy_quota_alert_enabled ?? false,
      );
      setCodebuddyQuotaAlertThreshold(
        String(config.codebuddy_quota_alert_threshold ?? 20),
      );
      setCodebuddyCnQuotaAlertEnabled(
        config.codebuddy_cn_quota_alert_enabled ?? false,
      );
      setCodebuddyCnQuotaAlertThreshold(
        String(config.codebuddy_cn_quota_alert_threshold ?? 20),
      );
      setWorkbuddyQuotaAlertEnabled(
        config.workbuddy_quota_alert_enabled ?? false,
      );
      setWorkbuddyQuotaAlertThreshold(
        String(config.workbuddy_quota_alert_threshold ?? 20),
      );
      setQoderQuotaAlertEnabled(config.qoder_quota_alert_enabled ?? false);
      setQoderQuotaAlertThreshold(
        String(config.qoder_quota_alert_threshold ?? 20),
      );
      setTraeQuotaAlertEnabled(config.trae_quota_alert_enabled ?? false);
      setTraeQuotaAlertThreshold(
        String(config.trae_quota_alert_threshold ?? 20),
      );
  ...
  const isAppPathResetDetecting = (target: AppPathTarget) =>
    appPathResetDetectingTargets.has(target);
  ...
  const windsurfAutoRefreshIsPreset =
    REFRESH_PRESET_VALUES.includes(windsurfAutoRefresh);
  const kiroAutoRefreshIsPreset =
    REFRESH_PRESET_VALUES.includes(kiroAutoRefresh);
  const codebuddyAutoRefreshIsPreset =
    REFRESH_PRESET_VALUES.includes(codebuddyAutoRefresh);
  const codebuddyCnAutoRefreshIsPreset = REFRESH_PRESET_VALUES.includes(
    codebuddyCnAutoRefresh,
  );
  const workbuddyAutoRefreshIsPreset =
    REFRESH_PRESET_VALUES.includes(workbuddyAutoRefresh);
  const qoderAutoRefreshIsPreset =
    REFRESH_PRESET_VALUES.includes(qoderAutoRefresh);
  const traeAutoRefreshIsPreset =
    REFRESH_PRESET_VALUES.includes(traeAutoRefresh);
  const zedAutoRefreshIsPreset = REFRESH_PRESET_VALUES.includes(zedAutoRefresh);
  const cursorAutoRefreshIsPreset =
    REFRESH_PRESET_VALUES.includes(cursorAutoRefresh);
  const geminiAutoRefreshIsPreset =
    REFRESH_PRESET_VALUES.includes(geminiAutoRefresh);
  const autoSwitchThresholdIsPreset =
    THRESHOLD_PRESET_VALUES.includes(autoSwitchThreshold);
  const quotaAlertThresholdIsPreset =
    THRESHOLD_PRESET_VALUES.includes(quotaAlertThreshold);
  const codexQuotaAlertThresholdIsPreset = THRESHOLD_PRESET_VALUES.includes(
    codexQuotaAlertThreshold,
  );
  const ghcpQuotaAlertThresholdIsPreset = THRESHOLD_PRESET_VALUES.includes(
    ghcpQuotaAlertThreshold,
  );
  const windsurfQuotaAlertThresholdIsPreset = THRESHOLD_PRESET_VALUES.includes(
    windsurfQuotaAlertThreshold,
  );
  const kiroQuotaAlertThresholdIsPreset = THRESHOLD_PRESET_VALUES.includes(
    kiroQuotaAlertThreshold,
  );
  const codebuddyQuotaAlertThresholdIsPreset = THRESHOLD_PRESET_VALUES.includes(
    codebuddyQuotaAlertThreshold,
  );
  const codebuddyCnQuotaAlertThresholdIsPreset =
    THRESHOLD_PRESET_VALUES.includes(codebuddyCnQuotaAlertThreshold);
  const workbuddyQuotaAlertThresholdIsPreset = THRESHOLD_PRESET_VALUES.includes(
    workbuddyQuotaAlertThreshold,
  );
  const qoderQuotaAlertThresholdIsPreset = THRESHOLD_PRESET_VALUES.includes(
    qoderQuotaAlertThreshold,
  );
  const traeQuotaAlertThresholdIsPreset = THRESHOLD_PRESET_VALUES.includes(
    traeQuotaAlertThreshold,
  );
  const zedQuotaAlertThresholdIsPreset = THRESHOLD_PRESET_VALUES.includes(
    zedQuotaAlertThreshold,
  );
  const cursorQuotaAlertThresholdIsPreset = THRESHOLD_PRESET_VALUES.includes(
    cursorQuotaAlertThreshold,
  );
  const geminiQuotaAlertThresholdIsPreset = THRESHOLD_PRESET_VALUES.includes(
    geminiQuotaAlertThreshold,
  );
  ...
  return (
    <main className="main-content">
            ...
            <>
              ...
              <div className="settings-group">
                <div className="settings-row">
                  ...
                  <div className="row-control">
                    <select
                      ...
                      value={language}
                      onChange={(e) =>
                        setLanguage(normalizeLanguage(e.target.value))
                      }
                    >
                ...
                <div className="settings-row">
                  <div className="row-label">
                    <div className="row-title">
                      {t("settings.general.sideNavLayout", "侧边栏布局")}
                    </div>
                    <div className="row-desc">
                      {t(
                        "settings.general.sideNavLayoutDesc",
                        "切换原始布局或经典布局",
                      )}
                    </div>
                  </div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setSideNavLayoutMode(
                          e.target.value as SideNavLayoutMode,
                        )
                      }
                    >
                      <option value="original">
                        {t(
                          "settings.general.sideNavLayoutOriginal",
                          "原始布局",
                        )}
                      </option>
                      <option value="classic">
                        {t("settings.general.sideNavLayoutClassic", "经典布局")}
                      </option>
                    </select>
                  </div>
                </div>
                    ...
                    >
                      {UI_SCALE_OPTIONS.map((value) => (
                        <option
                          key={value}
                          value={value}
                        >{`${Math.round(Number.parseFloat(value) * 100)}%`}</option>
                      ))}
                    </select>
                  </div>
                </div>
                ...
                <div className="settings-row">
                  ...
                  <div className="row-control">
                    <select
                      ...
                      onChange={(e) =>
                        setCloseBehavior(
                          e.target.value as "ask" | "minimize" | "quit",
                        )
                      }
                    >
                      <option value="ask">
                        {t("settings.general.closeBehaviorAsk")}
                      </option>
                      <option value="minimize">
                        {t("settings.general.closeBehaviorMinimize")}
                      </option>
                      <option value="quit">
                ...
                <div className="settings-row">
                  ...
                  <div className="row-control">
                    ...
                    >
                      <option value="false">
                        {t("settings.general.autoUpdateOff")}
                      </option>
                      <option value="true">
                        {t("settings.general.autoUpdateOn")}
                      </option>
                    </select>
                  </div>
                </div>
                ...
                <div className="settings-row">
                  <div className="row-label">
                    <div className="row-title">
                      {t("settings.general.updateReminder")}
                    </div>
                    <div className="row-desc">
                      {t("settings.general.updateReminderDesc")}
                    </div>
                  </div>
                  <div className="row-control">
                    <select
                    ...
                    >
                      <option value="true">
                        {t("settings.general.updateReminderOn")}
                      </option>
                      <option value="false">
                        {t("settings.general.updateReminderOff")}
                      </option>
                    </select>
                  </div>
                </div>
                  ...
                  <div className="settings-row">
                    ...
                    <div className="row-control">
                      <select
                        ...
                        onChange={(e) =>
                          setHideDockIcon(e.target.value === "true")
                        }
                      >
                        <option value="false">
                          {t(
                            "settings.general.hideDockIconOff",
                            "否（显示Dock图标）",
                          )}
                        </option>
                        <option value="true">
                          {t(
                            "settings.general.hideDockIconOn",
                            "是（隐藏Dock图标）",
                          )}
                        </option>
                      </select>
                ...
                <div className="settings-row">
                  <div className="row-label">
                    <div className="row-title">
                      {t(
                        "settings.general.floatingCardStartup",
                        "启动时显示悬浮卡片",
                      )}
                    </div>
                    <div className="row-desc">
                      {t(
                        "settings.general.floatingCardStartupDesc",
                        "应用启动后默认展示悬浮账号卡片",
                      )}
                    </div>
                  </div>
                  <div className="row-control">
                    <select
                      ...
                      onChange={(e) =>
                        setFloatingCardShowOnStartup(e.target.value === "true")
                      }
                    >
                ...
                <div className="settings-row">
                  <div className="row-label">
                    <div className="row-title">
                      {t(
                        "settings.general.floatingCardAlwaysOnTop",
                        "悬浮卡片默认置顶",
                      )}
                    </div>
                    <div className="row-desc">
                      {t(
                        "settings.general.floatingCardAlwaysOnTopDesc",
                        "新打开的悬浮卡片窗口默认保持置顶",
                      )}
                    </div>
                  </div>
                  <div className="row-control">
                    <select
                      ...
                      onChange={(e) =>
                        setFloatingCardAlwaysOnTop(e.target.value === "true")
                      }
                    >
                      <option value="false">
                        {t("common.disable", "停用")}
                ...
                <div className="settings-row">
                  <div className="row-label">
                    <div className="row-title">
                      {t(
                        "settings.general.floatingCardShowNow",
                        "立即显示悬浮卡片",
                      )}
                    </div>
                    <div className="row-desc">
                      {t(
                        "settings.general.floatingCardShowNowDesc",
                        "关闭后可在这里或托盘菜单中重新打开",
                      )}
                    </div>
                  </div>
                  <div className="row-control">
                    <button
                      ...
                      onClick={() => void showFloatingCardWindow()}
                    >
                      {t(
                ...
                <div className="settings-row">
                  ...
                  <div className="row-control">
                    <button
                      className="btn btn-secondary"
                      onClick={() => accountService.openDataFolder()}
                    >
                      <FolderOpen size={16} />
                      {t("common.open")}
                    </button>
                ...
                <div className="settings-row">
                  <div className="row-label">
                    <div className="row-title">
                      {t("settings.general.fpDir")}
                    </div>
                    <div className="row-desc">
                      {t("settings.general.fpDirDesc")}
                    </div>
                  </div>
                  <div className="row-control">
                    <button
                      className="btn btn-secondary"
                      onClick={() => accountService.openDeviceFolder()}
                    >
                      <FolderOpen size={16} />
                      {t("common.open")}
                    </button>
              ...
              <div style={{ display: "flex", flexDirection: "column" }}>
                <div style={{ order: platformSettingsOrder.antigravity }}>
                  <div className="group-title">
                    {t(
                      "settings.general.antigravitySettingsTitle",
                      "Antigravity 设置",
                    )}
                  </div>
                  <div className="settings-group">
                        ...
                        >
                          {autoRefreshCustomMode ? (
                            ...
                            >
                              ...
                              <option value="-1" disabled={hasActiveResetTasks}>
                                {t("settings.general.autoRefreshDisabled")}
                              </option>
                              <option value="2">
                                2 {t("settings.general.minutes")}
                              </option>
                              <option value="5" disabled={hasActiveResetTasks}>
                                5 {t("settings.general.minutes")}
                              </option>
                              <option value="10" disabled={hasActiveResetTasks}>
                                10 {t("settings.general.minutes")}
                              </option>
                              <option value="15" disabled={hasActiveResetTasks}>
                                15 {t("settings.general.minutes")}
                              </option>
                              <option
                                value="custom"
                                disabled={hasActiveResetTasks}
                              >
                    ...
                    <div className="settings-row">
                      <div className="row-label">
                        <div className="row-title">
                          {t(
                            "settings.general.antigravityAppPath",
                            "Antigravity 启动路径",
                          )}
                        </div>
                        <div className="row-desc">
                          {t(
                            "settings.general.codexAppPathDesc",
                            "留空则使用默认路径",
                          )}
                        </div>
                      </div>
                      <div className="row-control row-control--grow">
                        ...
                        >
                          <input
                            ...
                            placeholder={t(
                              "settings.general.codexAppPathPlaceholder",
                              "默认路径",
                            )}
                            onChange={(e) =>
                              setAntigravityAppPath(e.target.value)
                            }
                          />
                          <button
                            className="btn btn-secondary"
                    ...
                    {autoSwitchEnabled && (
                      ...
                      >
                        <div className="row-label">
                          ...
                          <div className="row-desc">
                            {t(
                              "quickSettings.autoSwitch.thresholdDesc",
                              "任意模型配额低于此百分比时触发自动切号",
                            )}
                          </div>
                        </div>
                          ...
                          {autoSwitchThresholdCustomMode ? (
                            ...
                            >
                              {!autoSwitchThresholdIsPreset && (
                                <option value={autoSwitchThreshold}>
                                  {autoSwitchThreshold}%
                                </option>
                              )}
                    ...
                    <div className="settings-row">
                      <div className="row-label">
                        <div className="row-title">
                          {t("quickSettings.quotaAlert.enable", "超额预警")}
                        </div>
                        <div className="row-desc">
                          {t(
                      ...
                      <div className="row-control">
                        <label className="switch">
                          <input
                            ...
                            onChange={(e) =>
                              setQuotaAlertEnabled(e.target.checked)
                            }
                          />
                          <span className="slider"></span>
                        </label>
                    ...
                    {quotaAlertEnabled && (
                      ...
                      >
                        <div className="row-label">
                          <div className="row-title">
                            {t(
                              "quickSettings.quotaAlert.threshold",
                              "预警阈值",
                            )}
                          </div>
                          <div className="row-desc">
                            {t(
                              "quickSettings.quotaAlert.thresholdDesc",
                              "任意模型配额低于此百分比时触发预警",
                            )}
                          </div>
                        </div>
                          ...
                          {quotaAlertThresholdCustomMode ? (
                            ...
                            >
                              {!quotaAlertThresholdIsPreset && (
                                <option value={quotaAlertThreshold}>
                                  {quotaAlertThreshold}%
                                </option>
                              )}
                              <option value="0">0%</option>
                ...
                <div style={{ order: platformSettingsOrder.codex }}>
                  <div className="group-title">
                    {t("settings.general.codexSettingsTitle", "Codex 设置")}
                  </div>
                  <div className="settings-group">
                    <div className="settings-row">
                      <div className="row-label">
                    ...
                    <div className="settings-row">
                      <div className="row-label">
                        <div className="row-title">
                          {t("settings.general.codexAppPath", "Codex 启动路径")}
                        </div>
                        <div className="row-desc">
                          {t(
                            "settings.general.codexAppPathDesc",
                    ...
                    <div className="settings-row">
                      <div className="row-label">
                        <div className="row-title">
                          {t("settings.general.opencodeAuthOverwrite")}
                        </div>
                        <div className="row-desc">
                          {t("settings.general.opencodeAuthOverwriteDesc")}
                        </div>
                      </div>
                      <div className="row-control">
                        <label className="switch">
                    ...
                    <div className="settings-row">
                      <div className="row-label">
                        <div className="row-title">
                          {t("settings.general.openclawAuthOverwrite")}
                        </div>
                        <div className="row-desc">
                          {t("settings.general.openclawAuthOverwriteDesc")}
                        </div>
                      </div>
                      <div className="row-control">
                        <label className="switch">
                          <input
                            ...
                            onChange={(e) =>
                              setOpenclawAuthOverwriteOnSwitch(e.target.checked)
                            }
                          />
                          <span className="slider"></span>
                        </label>
                    ...
                    <div className="settings-row">
                      <div className="row-label">
                        <div className="row-title">
                          {t("settings.general.opencodeRestart")}
                        </div>
                        <div className="row-desc">
                          {opencodeAuthOverwriteOnSwitch
                            ? t("settings.general.opencodeRestartDesc")
                            : t(
                                "settings.general.opencodeRestartRequiresOverwrite",
                              )}
                        </div>
                      </div>
                      <div className="row-control">
                        <label className="switch">
                          <input
                            ...
                            onChange={(e) =>
                              setOpencodeSyncOnSwitch(e.target.checked)
                            }
                    ...
                    <div className="settings-row">
                      <div className="row-label">
                        <div className="row-title">
                          {t("settings.general.opencodeAppPath")}
                        </div>
                        <div className="row-desc">
                      ...
                      <div className="row-control row-control--grow">
                        ...
                        >
                          <input
                            ...
                            placeholder={t(
                              "settings.general.opencodeAppPathPlaceholder",
                            )}
                      ...
                      <div className="row-control">
                        <label className="switch">
                          <input
                            ...
                            onChange={(e) =>
                              setCodexQuotaAlertEnabled(e.target.checked)
                            }
                          />
                          <span className="slider"></span>
                        </label>
                          ...
                          {codexQuotaAlertThresholdCustomMode ? (
                            ...
                            >
                              {!codexQuotaAlertThresholdIsPreset && (
                                <option value={codexQuotaAlertThreshold}>
                                  {codexQuotaAlertThreshold}%
                                </option>
                              )}
                              <option value="0">0%</option>
                ...
                <div style={{ order: platformSettingsOrder["github-copilot"] }}>
                  <div className="group-title">
                    {t(
                      "settings.general.githubCopilotSettingsTitle",
                      "GitHub Copilot 设置",
                    )}
                  </div>
                  <div className="settings-group">
                    <div className="settings-row">
                      <div className="row-label">
                        <div className="row-title">
                          {t(
                            "settings.general.ghcpAutoRefresh",
                            "GitHub Copilot 自动刷新配额",
                          )}
                        </div>
                        <div className="row-desc">
                          {t(
                            "settings.general.ghcpAutoRefreshDesc",
                            "后台自动更新频率",
                          )}
                        </div>
                      </div>
                    ...
                    <div className="settings-row">
                      <div className="row-label">
                        <div className="row-title">
                          {t(
                            "settings.general.vscodeAppPath",
                            "VS Code 启动路径",
                          )}
                        </div>
                        <div className="row-desc">
                          {t(
                            "settings.general.vscodeAppPathDesc",
                            "留空则使用默认路径",
                          )}
                        </div>
                      </div>
                      <div className="row-control row-control--grow">
                        ...
                        >
                          <input
                            ...
                            value={vscodeAppPath}
                            placeholder={t(
                              "settings.general.vscodeAppPathPlaceholder",
                              "默认路径",
                            )}
                      ...
                      <div className="row-control">
                        <label className="switch">
                          <input
                            ...
                            onChange={(e) =>
                              setGhcpQuotaAlertEnabled(e.target.checked)
                            }
                          />
                          <span className="slider"></span>
                        </label>
                          ...
                          {ghcpQuotaAlertThresholdCustomMode ? (
                            ...
                            >
                              {!ghcpQuotaAlertThresholdIsPreset && (
                                <option value={ghcpQuotaAlertThreshold}>
                                  {ghcpQuotaAlertThreshold}%
                                </option>
                              )}
                              <option value="0">0%</option>
                ...
                <div style={{ order: platformSettingsOrder.windsurf }}>
                  <div className="group-title">
                    {t(
                      "settings.general.windsurfSettingsTitle",
                      "Windsurf 设置",
                    )}
                  </div>
                  <div className="settings-group">
                    <div className="settings-row">
                      <div className="row-label">
                        <div className="row-title">
                          {t(
                            "settings.general.windsurfAutoRefresh",
                            "Windsurf 自动刷新配额",
                          )}
                        </div>
                        <div className="row-desc">
                          {t(
                            "settings.general.windsurfAutoRefreshDesc",
                            "后台自动更新频率",
                          )}
                        </div>
                      </div>
                    ...
                    <div className="settings-row">
                      <div className="row-label">
                        <div className="row-title">
                          {t(
                            "settings.general.windsurfAppPath",
                            "Windsurf 启动路径",
                          )}
                        </div>
                        <div className="row-desc">
                          {t(
                            "settings.general.windsurfAppPathDesc",
                            "留空则使用默认路径",
                          )}
                        </div>
                      </div>
                      <div className="row-control row-control--grow">
                        ...
                        >
                          <input
                            ...
                            placeholder={t(
                              "settings.general.windsurfAppPathPlaceholder",
                              "默认路径",
                            )}
                      ...
                      <div className="row-control">
                        <label className="switch">
                          <input
                            ...
                            onChange={(e) =>
                              setWindsurfQuotaAlertEnabled(e.target.checked)
                            }
                          />
                          <span className="slider"></span>
                        </label>
                          ...
                          {windsurfQuotaAlertThresholdCustomMode ? (
                            ...
                            <select
                              ...
                              onChange={(e) => {
                                ...
                                if (val === "custom") {
                                  ...
                                  setWindsurfQuotaAlertThreshold(
                                    windsurfQuotaAlertThreshold || "20",
                                  );
                                  return;
                                }
                                setWindsurfQuotaAlertThresholdCustomMode(false);
                            ...
                            >
                              {!windsurfQuotaAlertThresholdIsPreset && (
                                <option value={windsurfQuotaAlertThreshold}>
                                  {windsurfQuotaAlertThreshold}%
                                </option>
                              )}
                              <option value="0">0%</option>
                ...
                <div style={{ order: platformSettingsOrder.kiro }}>
                  <div className="group-title">
                    {t("settings.general.kiroSettingsTitle", "Kiro 设置")}
                  </div>
                  <div className="settings-group">
                    <div className="settings-row">
                      <div className="row-label">
                        <div className="row-title">
                          {t(
                            "settings.general.kiroAutoRefresh",
                            "Kiro 自动刷新配额",
                          )}
                        </div>
                        <div className="row-desc">
                          {t(
                            "settings.general.kiroAutoRefreshDesc",
                            "后台自动更新频率",
                          )}
                        </div>
                      </div>
                    ...
                    <div className="settings-row">
                      <div className="row-label">
                        <div className="row-title">
                          {t("settings.general.kiroAppPath", "Kiro 启动路径")}
                        </div>
                        <div className="row-desc">
                          {t(
                            "settings.general.kiroAppPathDesc",
                            "留空则使用默认路径",
                          )}
                        </div>
                      </div>
                      <div className="row-control row-control--grow">
                        ...
                        >
                          <input
                            ...
                            value={kiroAppPath}
                            placeholder={t(
                              "settings.general.kiroAppPathPlaceholder",
                              "默认路径",
                            )}
                      ...
                      <div className="row-control">
                        <label className="switch">
                          <input
                            ...
                            onChange={(e) =>
                              setKiroQuotaAlertEnabled(e.target.checked)
                            }
                          />
                          <span className="slider"></span>
                        </label>
                          ...
                          {kiroQuotaAlertThresholdCustomMode ? (
                            ...
                            >
                              {!kiroQuotaAlertThresholdIsPreset && (
                                <option value={kiroQuotaAlertThreshold}>
                                  {kiroQuotaAlertThreshold}%
                                </option>
                              )}
                              <option value="0">0%</option>
                ...
                <div style={{ order: platformSettingsOrder.codebuddy }}>
                  <div className="group-title">
                    {t(
                      "settings.general.codebuddySettingsTitle",
                      "CodeBuddy 设置",
                    )}
                  </div>
                  <div className="settings-group">
                    <div className="settings-row">
                      <div className="row-label">
                        <div className="row-title">
                          {t(
                            "settings.general.codebuddyAutoRefresh",
                            "CodeBuddy 自动刷新配额",
                          )}
                        </div>
                        <div className="row-desc">
                          {t(
                            "settings.general.codebuddyAutoRefreshDesc",
                            "后台自动更新频率",
                          )}
                        </div>
                      </div>
                            ...
                            >
                              <input
                                ...
                                onBlur={() => {
                                  ...
                                  if (
                                    REFRESH_PRESET_VALUES.includes(normalized)
                                  ) {
                                    setCodebuddyAutoRefreshCustomMode(false);
                                  }
                    ...
                    <div className="settings-row">
                      <div className="row-label">
                        <div className="row-title">
                          {t(
                            "settings.general.codebuddyAppPath",
                            "CodeBuddy 启动路径",
                          )}
                        </div>
                        <div className="row-desc">
                          {t(
                            "settings.general.codebuddyAppPathDesc",
                            "留空则使用默认路径",
                          )}
                        </div>
                      </div>
                      <div className="row-control row-control--grow">
                        ...
                        >
                          <input
                            ...
                            placeholder={t(
                              "settings.general.codebuddyAppPathPlaceholder",
                              "默认路径",
                            )}
                            onChange={(e) =>
                              setCodebuddyAppPath(e.target.value)
                            }
                          />
                          <button
                            className="btn btn-secondary"
                      ...
                      <div className="row-control">
                        <label className="switch">
                          <input
                            ...
                            onChange={(e) =>
                              setCodebuddyQuotaAlertEnabled(e.target.checked)
                            }
                          />
                          <span className="slider"></span>
                        </label>
                          ...
                          {codebuddyQuotaAlertThresholdCustomMode ? (
                            <div className="settings-inline-input">
                              <input
                                ...
                                onBlur={() => {
                                  ...
                                  if (
                                    THRESHOLD_PRESET_VALUES.includes(normalized)
                                  ) {
                                    setCodebuddyQuotaAlertThresholdCustomMode(
                                      false,
                                    );
                            ...
                            <select
                              ...
                              onChange={(e) => {
                                ...
                                if (val === "custom") {
                                  ...
                                  setCodebuddyQuotaAlertThreshold(
                                    codebuddyQuotaAlertThreshold || "20",
                                  );
                                  return;
                                }
                                setCodebuddyQuotaAlertThresholdCustomMode(
                            ...
                            >
                              {!codebuddyQuotaAlertThresholdIsPreset && (
                                <option value={codebuddyQuotaAlertThreshold}>
                                  {codebuddyQuotaAlertThreshold}%
                                </option>
                              )}
                              <option value="0">0%</option>
                ...
                <div style={{ order: platformSettingsOrder.codebuddy_cn }}>
                  <div className="group-title">
                    {t(
                      "settings.general.codebuddyCnSettingsTitle",
                      "CodeBuddy CN 设置",
                    )}
                  </div>
                  <div className="settings-group">
                    <div className="settings-row">
                      <div className="row-label">
                        <div className="row-title">
                          {t(
                            "settings.general.codebuddyCnAutoRefresh",
                            "CodeBuddy CN 自动刷新配额",
                          )}
                        </div>
                        <div className="row-desc">
                          {t(
                            "settings.general.codebuddyCnAutoRefreshDesc",
                            "后台自动更新频率",
                          )}
                        </div>
                      </div>
                    ...
                    <div className="settings-row">
                      <div className="row-label">
                        <div className="row-title">
                          {t(
                            "settings.general.codebuddyCnAppPath",
                            "CodeBuddy CN 启动路径",
                          )}
                        </div>
                        <div className="row-desc">
                          {t(
                            "settings.general.codebuddyCnAppPathDesc",
                            "留空则使用默认路径",
                          )}
                        </div>
                      </div>
                      <div className="row-control row-control--grow">
                        ...
                        >
                          <input
                            ...
                            placeholder={t(
                              "settings.general.codebuddyCnAppPathPlaceholder",
                              "默认路径",
                            )}
                            onChange={(e) =>
                              setCodebuddyCnAppPath(e.target.value)
                            }
                          />
                          <button
                            className="btn btn-secondary"
                          ...
                          >
                            {t(
                              "settings.general.codebuddyCnPathSelect",
                              "选择",
                            )}
                          </button>
                          <button
                      ...
                      <div className="row-control">
                        <label className="switch">
                          <input
                            ...
                            onChange={(e) =>
                              setCodebuddyCnQuotaAlertEnabled(e.target.checked)
                            }
                          />
                          <span className="slider"></span>
                        </label>
                          ...
                          {codebuddyCnQuotaAlertThresholdCustomMode ? (
                            ...
                            <select
                              ...
                              onChange={(e) => {
                                ...
                                if (val === "custom") {
                                  ...
                                  setCodebuddyCnQuotaAlertThreshold(
                                    codebuddyCnQuotaAlertThreshold || "20",
                                  );
                                  return;
                                }
                                setCodebuddyCnQuotaAlertThresholdCustomMode(
                            ...
                            >
                              {!codebuddyCnQuotaAlertThresholdIsPreset && (
                                <option value={codebuddyCnQuotaAlertThreshold}>
                                  {codebuddyCnQuotaAlertThreshold}%
                                </option>
                              )}
                              <option value="0">0%</option>
                ...
                <div style={{ order: platformSettingsOrder.qoder }}>
                  <div className="group-title">
                    {t("quickSettings.qoder.title", "Qoder 设置")}
                  </div>
                  <div className="settings-group">
                    <div className="settings-row">
                      <div className="row-label">
                        <div className="row-title">
                          {t(
                            "settings.general.qoderAutoRefresh",
                            "Qoder 自动刷新配额",
                          )}
                        </div>
                        <div className="row-desc">
                          {t(
                            "settings.general.qoderAutoRefreshDesc",
                            "后台自动更新频率",
                          )}
                        </div>
                      </div>
                    ...
                    <div className="settings-row">
                      <div className="row-label">
                        <div className="row-title">
                          {t("settings.general.qoderAppPath", "Qoder 启动路径")}
                        </div>
                        <div className="row-desc">
                          {t(
                            "settings.general.qoderAppPathDesc",
                            "留空则使用默认路径",
                          )}
                        </div>
                      </div>
                      <div className="row-control row-control--grow">
                        ...
                        >
                          <input
                            ...
                            value={qoderAppPath}
                            placeholder={t(
                              "settings.general.qoderAppPathPlaceholder",
                              "默认路径",
                            )}
                      ...
                      <div className="row-control">
                        <label className="switch">
                          <input
                            ...
                            onChange={(e) =>
                              setQoderQuotaAlertEnabled(e.target.checked)
                            }
                          />
                          <span className="slider"></span>
                        </label>
                          ...
                          {qoderQuotaAlertThresholdCustomMode ? (
                            ...
                            >
                              {!qoderQuotaAlertThresholdIsPreset && (
                                <option value={qoderQuotaAlertThreshold}>
                                  {qoderQuotaAlertThreshold}%
                                </option>
                              )}
                              <option value="0">0%</option>
                ...
                <div style={{ order: platformSettingsOrder.trae }}>
                  <div className="group-title">
                    {t("quickSettings.trae.title", "Trae 设置")}
                  </div>
                  <div className="settings-group">
                    <div className="settings-row">
                      <div className="row-label">
                        <div className="row-title">
                          {t(
                            "settings.general.traeAutoRefresh",
                            "Trae 自动刷新配额",
                          )}
                        </div>
                        <div className="row-desc">
                          {t(
                            "settings.general.traeAutoRefreshDesc",
                            "后台自动更新频率",
                          )}
                        </div>
                      </div>
                    ...
                    <div className="settings-row">
                      <div className="row-label">
                        <div className="row-title">
                          {t("settings.general.traeAppPath", "Trae 启动路径")}
                        </div>
                        <div className="row-desc">
                          {t(
                            "settings.general.traeAppPathDesc",
                            "留空则使用默认路径",
                          )}
                        </div>
                      </div>
                      <div className="row-control row-control--grow">
                        ...
                        >
                          <input
                            ...
                            value={traeAppPath}
                            placeholder={t(
                              "settings.general.traeAppPathPlaceholder",
                              "默认路径",
                            )}
                      ...
                      <div className="row-control">
                        <label className="switch">
                          <input
                            ...
                            onChange={(e) =>
                              setTraeQuotaAlertEnabled(e.target.checked)
                            }
                          />
                          <span className="slider"></span>
                        </label>
                          ...
                          {traeQuotaAlertThresholdCustomMode ? (
                            ...
                            >
                              {!traeQuotaAlertThresholdIsPreset && (
                                <option value={traeQuotaAlertThreshold}>
                                  {traeQuotaAlertThreshold}%
                                </option>
                              )}
                              <option value="0">0%</option>
                ...
                <div style={{ order: platformSettingsOrder.workbuddy }}>
                  <div className="group-title">
                    {t("quickSettings.workbuddy.title", "WorkBuddy 设置")}
                  </div>
                  <div className="settings-group">
                    <div className="settings-row">
                      <div className="row-label">
                        <div className="row-title">
                          {t(
                            "settings.general.workbuddyAutoRefresh",
                            "WorkBuddy 自动刷新配额",
                          )}
                        </div>
                        <div className="row-desc">
                          {t(
                            "settings.general.workbuddyAutoRefreshDesc",
                            "后台自动更新频率",
                          )}
                        </div>
                      </div>
                    ...
                    <div className="settings-row">
                      <div className="row-label">
                        <div className="row-title">
                          {t(
                            "settings.general.workbuddyAppPath",
                            "WorkBuddy 启动路径",
                          )}
                        </div>
                        <div className="row-desc">
                          {t(
                            "settings.general.workbuddyAppPathDesc",
                            "留空则使用默认路径",
                          )}
                        </div>
                      </div>
                      <div className="row-control row-control--grow">
                        ...
                        >
                          <input
                            ...
                            placeholder={t(
                              "settings.general.workbuddyAppPathPlaceholder",
                              "默认路径",
                            )}
                            onChange={(e) =>
                              setWorkbuddyAppPath(e.target.value)
                            }
                          />
                          <button
                            className="btn btn-secondary"
                      ...
                      <div className="row-control">
                        <label className="switch">
                          <input
                            ...
                            onChange={(e) =>
                              setWorkbuddyQuotaAlertEnabled(e.target.checked)
                            }
                          />
                          <span className="slider"></span>
                        </label>
                          ...
                          {workbuddyQuotaAlertThresholdCustomMode ? (
                            ...
                            <select
                              ...
                              onChange={(e) => {
                                ...
                                if (val === "custom") {
                                  ...
                                  setWorkbuddyQuotaAlertThreshold(
                                    workbuddyQuotaAlertThreshold || "20",
                                  );
                                  return;
                                }
                                setWorkbuddyQuotaAlertThresholdCustomMode(
                            ...
                            >
                              {!workbuddyQuotaAlertThresholdIsPreset && (
                                <option value={workbuddyQuotaAlertThreshold}>
                                  {workbuddyQuotaAlertThreshold}%
                                </option>
                              )}
                              <option value="0">0%</option>
                ...
                <div style={{ order: platformSettingsOrder.zed }}>
                  <div className="group-title">
                    {t("quickSettings.zed.title", "Zed 设置")}
                  </div>
                  <div className="settings-group">
                    <div className="settings-row">
                      <div className="row-label">
                        <div className="row-title">
                          {t(
                            "settings.general.zedAutoRefresh",
                            "Zed 自动刷新配额",
                          )}
                        </div>
                        <div className="row-desc">
                          {t(
                            "settings.general.zedAutoRefreshDesc",
                            "后台自动更新频率",
                          )}
                        </div>
                      </div>
                    ...
                    <div className="settings-row">
                      <div className="row-label">
                        <div className="row-title">
                          {t("settings.general.zedAppPath", "Zed 启动路径")}
                        </div>
                        <div className="row-desc">
                          {t(
                            "settings.general.zedAppPathDesc",
                            "留空则使用默认路径",
                          )}
                        </div>
                      </div>
                      <div className="row-control row-control--grow">
                        ...
                        >
                          <input
                            ...
                            value={zedAppPath}
                            placeholder={t(
                              "settings.general.zedAppPathPlaceholder",
                              "默认路径",
                            )}
                      ...
                      <div className="row-control">
                        <label className="switch">
                          <input
                            ...
                            onChange={(e) =>
                              setZedQuotaAlertEnabled(e.target.checked)
                            }
                          />
                          <span className="slider"></span>
                        </label>
                          ...
                          {zedQuotaAlertThresholdCustomMode ? (
                            ...
                            >
                              {!zedQuotaAlertThresholdIsPreset && (
                                <option value={zedQuotaAlertThreshold}>
                                  {zedQuotaAlertThreshold}%
                                </option>
                              )}
                              <option value="0">0%</option>
                ...
                <div style={{ order: platformSettingsOrder.cursor }}>
                  <div className="group-title">
                    {t("quickSettings.cursor.title", "Cursor 设置")}
                  </div>
                  <div className="settings-group">
                    <div className="settings-row">
                      <div className="row-label">
                        <div className="row-title">
                          {t(
                            "quickSettings.cursorRefreshInterval",
                            "配额自动刷新",
                          )}
                        </div>
                        <div className="row-desc">
                      ...
                      <div className="row-control">
                        <label className="switch">
                          <input
                            ...
                            onChange={(e) =>
                              setCursorQuotaAlertEnabled(e.target.checked)
                            }
                          />
                          <span className="slider"></span>
                        </label>
                          ...
                          {cursorQuotaAlertThresholdCustomMode ? (
                            ...
                            >
                              {!cursorQuotaAlertThresholdIsPreset && (
                                <option value={cursorQuotaAlertThreshold}>
                                  {cursorQuotaAlertThreshold}%
                                </option>
                              )}
                              <option value="0">0%</option>
                ...
                <div style={{ order: platformSettingsOrder.gemini }}>
                  <div className="group-title">
                    {t("quickSettings.gemini.title", "Gemini Cli 设置")}
                  </div>
                  <div className="settings-group">
                    <div className="settings-row">
                      <div className="row-label">
                        <div className="row-title">
                          {t(
                            "quickSettings.geminiRefreshInterval",
                            "配额自动刷新",
                          )}
                        </div>
                        <div className="row-desc">
                      ...
                      <div className="row-control">
                        <label className="switch">
                          <input
                            ...
                            onChange={(e) =>
                              setGeminiQuotaAlertEnabled(e.target.checked)
                            }
                          />
                          <span className="slider"></span>
                        </label>
                          ...
                          {geminiQuotaAlertThresholdCustomMode ? (
                            ...
                            >
                              {!geminiQuotaAlertThresholdIsPreset && (
                                <option value={geminiQuotaAlertThreshold}>
                                  {geminiQuotaAlertThreshold}%
                                </option>
                              )}
                              <option value="0">0%</option>
            ...
            <>
              ...
              <div className="settings-group">
                <div className="settings-row">
                  <div className="row-label">
                    <div className="row-title">
                      {t("settings.network.wsService")}
                    </div>
                    <div className="row-desc">
                      {t("settings.network.wsServiceDesc")}
                    </div>
                  </div>
                  <div className="row-control">
                    <label className="switch">
                ...
                {wsEnabled && (
                  <>
                    ...
                    >
                      <div className="row-label">
                        <div className="row-title">
                          {t("settings.network.preferredPort")}
                        </div>
                        <div className="row-desc">
                          {t("settings.network.preferredPortDesc").replace(
                            "{port}",
                            String(defaultPort),
                          )}
                        </div>
                    ...
                    {actualPort && (
                      ...
                      >
                        <div className="row-label">
                          <div className="row-title">
                            {t("settings.network.currentPort")}
                          </div>
                          <div className="row-desc">
              ...
              <div className="group-title">
                {t("settings.network.reportTitle")}
              </div>
              <div className="settings-group">
                <div className="settings-row">
                  <div className="row-label">
                    <div className="row-title">
                      {t("settings.network.reportService")}
                    </div>
                    <div className="row-desc">
                      {t("settings.network.reportServiceDesc")}
                    </div>
                  </div>
                  <div className="row-control">
                    <label className="switch">
                ...
                {reportEnabled && (
                  <>
                    ...
                    >
                      <div className="row-label">
                        <div className="row-title">
                          {t("settings.network.reportPort")}
                        </div>
                        <div className="row-desc">
                          {t("settings.network.reportPortDesc").replace(
                            "{port}",
                            String(reportDefaultPort),
                          )}
                        </div>
                      </div>
                      <div className="row-control">
                    ...
                    >
                      <div className="row-label">
                        <div className="row-title">
                          {t("settings.network.reportToken")}
                        </div>
                        <div className="row-desc">
                          {t("settings.network.reportTokenDesc")}
                        </div>
                      </div>
                      <div
                    ...
                    >
                      <div className="row-label">
                        <div className="row-title">
                          {t("settings.network.reportUrlPreview")}
                        </div>
                        <div className="row-desc">
                    ...
                    >
                      <div className="row-label">
                        <div className="row-title">
                          {t("settings.network.firewallHintTitle")}
                        </div>
                        <div className="row-desc">
                          {t("settings.network.firewallHint")}
                        </div>
                      </div>
                    </div>
                  </>
              ...
              <div className="settings-group">
                <div className="settings-row">
                  <div className="row-label">
                    ...
                    <div className="row-desc">
                      {t("settings.network.proxyEnabledDesc")}
                    </div>
                  </div>
                  <div className="row-control">
                    <label className="switch">
                      <input
                        ...
                        onChange={(e) =>
                          setGlobalProxyEnabled(e.target.checked)
                        }
                      />
                      <span className="slider"></span>
                    </label>
                  ...
                  <>
                    ...
                    >
                      <div className="row-label">
                        ...
                        <div className="row-desc">
                          {t("settings.network.proxyUrlDesc")}
                        </div>
                      </div>
                      <div className="row-control">
                        <input
                          ...
                          placeholder={t(
                            "settings.network.proxyUrlPlaceholder",
                          )}
                        />
                      </div>
                    </div>
                    ...
                    >
                      <div className="row-label">
                        <div className="row-title">
                          {t("settings.network.proxyNoProxy")}
                        </div>
                        <div className="row-desc">
                          {t("settings.network.proxyNoProxyDesc")}
                        </div>
                      </div>
                      <div className="row-control">
                        <input
                          ...
                          onChange={(e) =>
                            setGlobalProxyNoProxy(e.target.value)
                          }
                          placeholder={t(
                            "settings.network.proxyNoProxyPlaceholder",
                          )}
                        />
                      </div>
                    </div>
            ...
            <div className="about-container">
              ...
              <div className="credits-list">
                <button
                  ...
                  onClick={() => openLink("https://github.com/jlcodes99")}
                >
                ...
                <button
                  className="credit-item"
                  onClick={() =>
                    openLink("https://github.com/jlcodes99/cockpit-tools")
                  }
                >
                  <div className="credit-icon" style={{ color: "#0f172a" }}>
                    <Github size={24} />
                  </div>
                ...
                <button
                  ...
                  onClick={() =>
                    openLink(
                      "https://github.com/jlcodes99/cockpit-tools/blob/main/docs/DONATE.md",
                    )
                  }
                >
                  <div className="credit-icon" style={{ color: "#ef4444" }}>
                    <Heart size={24} />
                  </div>
                ...
                >
                  <div className="credit-icon" style={{ color: "#3b82f6" }}>
      ...
      {showUnlockFireworks && <UnlockFireworksOverlay />}
    </main>
  );
}
