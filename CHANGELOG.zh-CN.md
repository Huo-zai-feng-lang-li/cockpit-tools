...
- **跨平台发布构建修正**：补回 Antigravity Inspector 辅助类型与函数的 Windows 条件编译，恢复 Linux 和 macOS release 构建可编译性。
...
- **Codex 配额刷新节拍**: 拆分当前账号刷新和所有账号刷新，当前账号按自己的间隔刷新，所有账号定时器默认跳过当前账号。
- **切号开关隔离**: 将 Codex 插件无感换号开关与 Antigravity 的无感双通道切号（不重启）拆开持久化，两个设置互不影响。

## [0.20.57] - 2026-05-27
### 修复
- **更新日志嵌入路径**: 修正 `update_checker.rs` 中双语更新日志的静态嵌入路径，修复编译时路径找不到的问题。
- **双语更新说明兜底**: 在提取的版本更新说明为空时增加自动兜底机制，避免前端升级弹窗显示为空白。
- **更新日志提取单元测试**: 新增针对双语更新说明提取与兜底机制的单元测试。
...
- **项目发布规则**：新增项目规则，写清 CI、版本号、双语 changelog、tag 部署和发布后核验流程。

## [0.20.53] - 2026-05-27
### 新增
- **Codex 切号目标控制**：新增点击切号同步运行端总开关，并拆分为反重力 IDE 插件无感换号和桌面端 Codex App 重启换号两个核心入口。

### 变更
- **Codex 切号行为**：点击切号现在按用户选择的运行端执行，不再强行把插件端和桌面端流程绑在一起。
- **Codex 无感换号速度**：复用上次成功的 Antigravity Inspector 端点，并缩短运行时等待窗口，降低无感切号延迟。
...
## [0.20.47] - 2026-05-26
### 修复
- **跨平台编译错误**：修复了在非 Windows 平台（如 Linux/macOS）上因缺少 `RuntimeTokens` 类型声明和相关辅助函数条件编译所导致的编译报错。
- **清理编译器警告**：修复并清理了多个模块（包括悬浮卡片窗口、进程管理、额度系统、Zed 账号等）中特定平台的未使用变量和死代码警告。

## [0.20.46] - 2026-05-26
### 新增
- **Codex Antigravity 插件无感切号**：通过 Antigravity Codex 插件运行时切换 Codex 账号，不重启、不写入 `auth.json`。

### 修复
- **Codex 额度唤醒合并**：保留精确额度窗口匹配，同时合入满额度兜底去重与下一次运行时间修复。

## [0.20.41] - 2026-05-25
### 修复
- **Codex 满额度唤醒去重**：账号已有任一有效额度重置倒计时时，不再因另一个窗口显示满额且缺少 `reset_time` 被满额度兜底重复唤醒。

## [0.20.40] - 2026-05-24
### 新增
- **Codex 桌面端唤醒运行时**：Codex 唤醒任务现在可自动发现桌面端内置运行时，不再只依赖系统 `codex` CLI。

### 修复
- **Codex 周额度满额唤醒**：修复 Free 账号周额度显示为 7d/满额但缺少 `reset_time` 时不触发的问题；任务选中的号池账号会按账号独立唤醒并写入冷却，避免重复唤醒。

## [0.20.39] - 2026-05-11
### 修复
- **无感切号稳定性**：将等待扩展响应的超时时间从 12 秒提升至 25 秒，解决了在弱网环境或宿主 IDE 负载较高时偶发的响应超时报错问题。
...
- **唤醒调度逻辑实现状态全持久化**：Antigravity 与 Codex 的唤醒调度器现已将其关键运行时状态（`last_run_at`、`reset_states`、`last_executed_at`）持久化至本地磁盘。这确保了应用重启不再会导致对当前周期内已处理账号产生冗余的“重复唤醒波”。
- **精细化账号级定向唤醒**：重构了唤醒触发机制，使其按个体账号而非任务整体运行。现在，当特定账号配额重置时，系统仅会唤醒该目标账号，避免了非必要的全量批处理执行。
...
- **无感切号 UI 默认开放**：移除了"无感双通道切号"与"切换记录"的隐藏式功能解锁门控，两项功能现在对所有用户直接可见，无需在关于页连点图标。
...
- **Tooltip 文案闭环**：完善了“速率预判水位”的交互说明，现在进一步明确：只有当配额低于此百分比时，系统不仅启动速率预判，同时也会在这个水位段开启高频“加速盯梢”。
...
- **Tooltip 图标质感优化**：将原本橘黄色倾斜的 `!` 警告感叹号优化为具备极客美感的 `Tip` 胶囊徽章。去除了杂乱感的斜体，采用了科技感蓝紫高阶渐变与 3D 高光内外阴影设计，质感进一步拉满。
...
- **弹窗 Tooltip 截断问题**：修复了在“快速设置”面板中，由于左侧无缓冲边界导致超长说明文案（如切号阈值防线兜底说明）被溢出截断的问题。优化了弹框绝对定位锚点。
...
- **速率预判水位置于用户态配置**：新增自动切号界面速率预判激活水位的调整功能，允许用户自行调整速率预判的启动时机（默认从 15% 介入），提供更极客的换号控制。

### 变更

- **切号原因多维度降级渲染**：切号历史中的自动切号原因现在升级为多行 JSX 组件展示（包含命中规则、触发范围、限制模型组与备选账户）。
- **切号安全防线机制说明进阶**：ToolTip 新增“不会过早切”与“不会漏切”的双重防线兜底承诺。
- **速率预判默认拦截线释放**：底层的速率预判预切换门槛由 30% 落至 15%，减少高比例可用健康区段的错判中断，进一步榨干单账号利益。

### 修复

- **速率预判机制 UI 缺漏**：修复在触发底层速率引擎提前拦截时，切号历史规则回显为“未知”的问题，现已补全映射为“速率预判提前拦截”。
...
- **自动切号机制说明详细化**：重新优化并展开了自动切号触发设置旁的 Tip。现已详细分层展示 4 重智能防御体系（动态侦测提频、衰减速率预判、紧急熔断恢复、用户阈值兜底），让用户更透明地掌握底层调度逻辑。
...
- **防呆设置与展示优化**：全局刷新设置限制为 3/5/10 分钟，调整相关文案以精简化方式澄清“全局刷新”与“局部切号探索雷达”之间的隔离关系。
- **默认兜底阈值更改**：由于智能预测与熔断抓取已足够可靠，默认切号阈值从 20% 降至 0%，推荐榨干每滴配额。
...
- **账号页搜索框现已支持一键清空机制**：给包含内容的搜索框内联新增了 'X' 清理动作，点击可立即重置搜索输入，优化跨账号快速过滤的连贯体验。
- **账号页搜索框现已加宽并支持自适应展示**：放宽了 Antigravity、Codex、Kiro、Windsurf 等平台账号页的搜索框宽度限制 (`clamp(160px, 14vw, 220px)`) 以更好地承载正常的邮箱输入长度，同时补齐了按钮区内边距防止文本相互遮挡。

### 变更

- **Antigravity 无感切号底层架构现已全盘整理文档化**：更新了 `无感切号原理.md`，完整记录了双通道传输模型、IDE 宿主接口内存热替换原理以及本版本建立的混合动力四层防御体系（速率预判、阈值兜底、紧急反应与动态提频）。

### 修复

- **无感切号链路现已在所有成功和重试失败路径中稳定通知前端**：修复了 `switch_account_dual_no_restart` 内 WebSocket 消息通知丢失的 bug，确保切号在插件端执行完成或中断时，Tauri 界面都能立刻触发配额状态回刷。
...
- **Antigravity 唤醒链路现已默认使用 legacy_cloudcode 直连模式，绕过 Language Server 冷启动**：唤醒请求不再经过 client_gateway 级联拉起官方 LS 进程，消除了 60 秒以上的冷启动超时问题，实现秒级响应。
- **唤醒链路 HTTP 错误（如 429 配额超限）现已被正确格式化为前端可渲染的 UI Payload**：新增 `encode_http_error_to_ui_payload` 适配器，将 Google 返回的原始错误 JSON 解构并封装为 `WakeupUiErrorPayload`，确保界面能优雅展示"配额超限"或"需验证"等提示，而非裸 JSON。
- **配额刷新现已对 429 Too Many Requests 立即返回，不再触发无效重试与休眠**：`loadCodeAssist` 与 `fetchAvailableModels` 在收到 429 时直接短路返回配额结果，避免对已限流账号执行指数退避重试，大幅提升"全部刷新"批量操作速度。
