...
## [0.20.44] - 2026-05-25
### 修复
- **Codex 满额度唤醒去重**：账号完成一次额度重置唤醒后，会按当前配额窗口时长进入冷却，避免服务端重置时间在轮询间漂移时重复唤醒同一账号。

## [0.20.43] - 2026-05-25
### 修复
- **Codex 满额度唤醒调度**：额度重置任务会保留即将到来的 7d 窗口起点作为 `next_run_at` 候选，但只有真正到达周期起点后才执行，避免 60 秒新鲜窗口内时序差导致重复唤醒。

## [0.20.42] - 2026-05-25
### 修复
- **Codex 7d 满额度唤醒判定**：仅在任务号池账号处于完整 7d 重置窗口且额度达到阈值时触发唤醒，已进入 `6d 23h...` 倒计时的账号不再重复触发。
- **Codex 配额窗口识别**：兼容接口将 7d 窗口返回在 `primary_window` 字段的情况，`secondary_window` 唤醒任务仍能正确识别 7d 满额度账号。
- **Codex 新账号任务覆盖**：新增 OAuth Codex 账号会自动加入已启用的额度满唤醒任务，避免账号池和任务号池脱节。
