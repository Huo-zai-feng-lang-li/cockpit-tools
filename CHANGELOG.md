...
## [0.20.44] - 2026-05-25
### Fixed
- **Codex full-quota wakeup dedupe**: Accounts that already completed a quota-reset wakeup now stay disarmed for the active quota window duration, preventing repeated wakeups when the provider's reset timestamp drifts between scheduler polls.
...
- **Antigravity wakeup gateway now injects proxy environment variables into the official Language Server subprocess**: when the wakeup gateway spawns the official LS binary, it now applies configured global proxy settings via `apply_managed_proxy_env_to_command`, ensuring the LS process can reach Google Cloud Code upstream servers through proxied networks and resolving the 60-second startup timeout caused by missing proxy configuration in the LS subprocess.
