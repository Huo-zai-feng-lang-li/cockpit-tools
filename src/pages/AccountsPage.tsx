...
import {
  AccountGroupModal,
  AddToGroupModal,
} from "../components/AccountGroupModal";
...
import { getAntigravityQuotaDisplayItems } from "../presentation/platformAccountPresentation";
import {
...
} from "../components/FileCorruptedModal";
...
const buildVerificationHistoryMaps = (
  batches: VerificationHistoryBatch[] = [],
) => {
...
export function AccountsPage({ onNavigate }: AccountsPageProps) {
  ...
  >({});
  const [verificationDetailMap, setVerificationDetailMap] = useState<
    Record<string, VerificationDetailRecord>
  >({});
  ...
  const loadVerificationHistory = useCallback(async () => {
    ...
    try {
      const batches = await invoke<VerificationHistoryBatch[]>(
        "wakeup_verification_load_history",
      );
      if (verificationHistoryRequestIdRef.current !== requestId) {
        return;
      }
      const { statusMap, detailMap } = buildVerificationHistoryMaps(
        batches || [],
      );
  ...
  const getVerificationBadge = useCallback(
    (account: Account) => {
      ...
      const reason =
        account.disabled_reason || verificationStatusMap[account.id];
      if (reason === "verification_required") {
        return {
          label: t("wakeup.errorUi.verificationRequiredTitle", "Need Verify"),
          className: "is-warning",
        };
      }
      if (reason === "tos_violation") {
        return {
          label: t("wakeup.errorUi.tosViolationTitle", "TOS"),
          className: "is-tos-violation",
        };
      }
      return null;
    },
  ...
  const [switchHistoryClearConfirmOpen, setSwitchHistoryClearConfirmOpen] =
    useState(false);
  const [switchHistory, setSwitchHistory] = useState<
    AntigravitySwitchHistoryItem[]
  >([]);
  const [
  ...
  const [showVerificationErrorModal, setShowVerificationErrorModal] = useState<
    string | null
  >(null);
  ...
  const [groupAccountPickerGroupId, setGroupAccountPickerGroupId] = useState<
    string | null
  >(null);
  ...
  const groupAccountPickerGroup = useMemo(() => {
    ...
    return (
      accountGroups.find((group) => group.id === groupAccountPickerGroupId) ||
      null
    );
  ...
  const getAvailableAICreditsDisplay = (account: Account): string => {
    ...
    for (const credit of credits) {
      ...
      const parsed = Number.parseFloat(
        String(credit.credit_amount).replace(/,/g, "").trim(),
      );
  ...
  useEffect(() => {
    ...
    if (normalizedSortBy.startsWith(ANTIGRAVITY_RESET_SORT_PREFIX)) {
      const targetGroupId = normalizedSortBy.slice(
        ANTIGRAVITY_RESET_SORT_PREFIX.length,
      );
  ...
  const filteredAccounts = useMemo(() => {
    ...
    if (tagFilter.length > 0) {
      ...
      result = result.filter((acc) =>
        accountMatchesTagFilters(acc, selectedTags),
      );
    }
    ...
    listen("oauth-callback-received", async () => {
      ...
      try {
        ...
        if (activeGroupIdRef.current && newAccount?.id) {
          await assignAccountsToGroup(activeGroupIdRef.current, [
            newAccount.id,
          ]);
  ...
  const consumeExternalProviderImport = useCallback(() => {
    const request =
      consumeQueuedExternalProviderImportForPlatform("antigravity");
    if (!request) return;
    openAddModal("token");
  ...
  const handleSwitch = async (accountId: string) => {
    ...
    try {
      ...
      await fetchCurrentAccount();
      setMessage({
        text: t("messages.switched", { email: maskAccountText(account.email) }),
      });
    } catch (e) {
      const raw = String(e);
  ...
  const formatSwitchHistoryAutoRule = (rule?: string | null) => {
    if (rule === "current_disabled") {
      return t(
        "accounts.switchHistory.autoReasonRuleCurrentDisabled",
        "当前账号已禁用",
      );
    }
    if (rule === "current_quota_forbidden") {
      return t(
        "accounts.switchHistory.autoReasonRuleQuotaForbidden",
        "当前账号配额受限",
      );
    }
    if (rule === "group_below_threshold") {
      return t(
        "accounts.switchHistory.autoReasonRuleGroupBelowThreshold",
        "模型分组低于阈值",
      );
    }
  ...
  const formatSwitchHistoryAutoScope = (scopeMode?: string | null) => {
    if (scopeMode === "selected_groups") {
      return t(
        "accounts.switchHistory.autoReasonScopeSelectedGroups",
        "指定模型分组",
      );
    }
  ...
  const handleImportFromLocal = async () => {
    ...
    try {
      ...
      setAddStatus("success");
      setAddMessage(
        t("messages.importLocalSuccess", {
          email: maskAccountText(imported.email),
        }),
      );
      setTimeout(() => {
        setShowAddModal(false);
  ...
  const handleImportFromFiles = async () => {
    ...
    try {
      ...
      unlistenProgress = await listen<{
        ...
        total: number;
        email: string;
      }>("accounts:file-import-progress", (event) => {
    ...
    if (importedAccounts.length > 0) {
      ...
      if (activeGroupId) {
        await assignAccountsToGroup(
          activeGroupId,
          importedAccounts.map((acc) => acc.id),
        );
        await reloadAccountGroups();
      }
    }
  ...
  const requestDeleteTag = (tag: string) => {
    ...
    const count = accounts.filter((account) =>
      (account.tags || []).some(
        (item) => normalizeAccountTag(item) === normalized,
      ),
    ).length;
    ...
    try {
      ...
      if (firstRejected) {
        setTagDeleteConfirmError(
          t("messages.actionFailed", {
            action: t("common.delete"),
            error: String(firstRejected.reason),
          }),
        );
        return;
      }
      ...
      setTagFilter((prev) =>
        prev.filter((item) => normalizeAccountTag(item) !== target),
      );
      await fetchAccounts();
  ...
  const handleAssignAccountsToGroup = async (
    ...
    const addedIds = accountIds.filter(
      (accountId) => !currentIds.has(accountId),
    );
    const removedIds = currentGroup.accountIds.filter(
      (accountId) => !nextIds.has(accountId),
    );
    const shouldRename = nextName !== currentGroup.name;

    if (!shouldRename && addedIds.length === 0 && removedIds.length === 0)
      return;
  ...
  const renderGridCards = (items: Account[], groupKey?: string) =>
    items.map((account) => {
      ...
      const hasVerificationIssue =
        verificationReason === "verification_required" ||
        verificationReason === "tos_violation";
        ...
        >
          <div className="card-top">
            ...
            {(() => {
              ...
              return vBadge ? (
                <span
                  className={`verification-status-pill ${vBadge.className}`}
                  title={vBadge.label}
                >
                  {vBadge.label}
                </span>
          ...
          {account.notes && (
            <div className="card-notes">
              <span className="notes-text" title={account.notes}>
                {account.notes}
              </span>
            </div>
          )}
          ...
          <div className="card-quota-grid">
            ...
            <div className="quota-credits-field">
              <span className="quota-credits-label">
                {t(
                  "common.shared.credits.availableAiCredits",
        ...
        >
          ...
          <div className="folder-inline-preview">
            {groupAccounts.map((acc) => (
              <div
                key={acc.id}
                className={`folder-preview-item${acc.disabled ? " disabled" : ""}`}
              >
                <span
                  className="folder-preview-email"
                  title={maskAccountText(acc.email) || ""}
                >
                  {maskAccountText(acc.email)}
                </span>
                {acc.quota?.subscription_tier && (
                  <span
                    className={`tier-badge ${(acc.quota.subscription_tier || "")
                      .replace(/-tier$/, "")
                      .replace("g1-", "")
                      .toLowerCase()}`}
                  >
  ...
  const renderCompactView = () => {
    ...
    const renderCompactCards = (items: Account[]) =>
      items.map((account) => {
        ...
        if (isDisabled)
          statusHints.push(disabledTitle || t("accounts.status.disabled"));
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      ...
                      onClick={(e) =>
                        openColorPicker(e, group.id, isPickerOpen)
                      }
                    >
                      <span
          ...
          {groupByTag ? (
            ...
            <>
              ...
              <div className={styles.grid}>
                {renderCompactCards(filteredAccounts)}
              </div>
            </>
          )}
        </div>
          ...
          <td>
            <div className="account-cell">
              ...
              <div className="account-sub-line">
                ...
                {isForbidden && (
                  <span
                    className="status-pill forbidden"
  ...
  return (
    <>
        ...
        <div className="toolbar">
          <div className="toolbar-left">
            ...
            <MultiSelectFilterDropdown
              ...
              onToggleValue={(value) =>
                toggleFilterTypeValue(value as AccountFilterType)
              }
                  ...
                  <div className="oauth-link">
                    ...
                    <div className="oauth-link-row oauth-manual-input">
                      ...
                      <button
                        ...
                        disabled={
                          !oauthCallbackInput.trim() || oauthCallbackSubmitting
                        }
                      >
          ...
          <div className="modal modal-lg" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              ...
              <button
                ...
                onClick={() => {
                  if (switchHistoryClearing || switchHistoryClearConfirmOpen)
                    return;
                  setShowSwitchHistoryModal(false);
                  setSwitchHistoryClearConfirmOpen(false);
                }}
                    ...
                    >
                      ...
                      <div style={{ fontWeight: 600, fontSize: 14 }}>
                        {t("accounts.switchHistory.target", {
                          email:
                            maskAccountText(item.targetEmail) ||
                            item.targetEmail ||
                            "-",
                      ...
                      >
                        {t("accounts.switchHistory.duration", {
                          ...
                          defaultValue:
                            "耗时：总 {{total}}ms，本地 {{local}}ms，无感 {{seamless}}ms",
                        })}
                      </div>
                      {!item.success && (
                        <div
          ...
          >
            <div className="modal" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="modal-body">
                <p>
                  {t(
                    "accounts.switchHistory.clearConfirm",
                    "确定清空全部切换记录吗？",
                  )}
                </p>
              </div>
            ...
            <div className="modal-body">
              ...
              <p>
                {t("accounts.confirmDeleteTag", {
                  ...
                  defaultValue:
                    '确认删除标签 "{{tag}}" 吗？该标签将从 {{count}} 个账号中移除。',
                })}
              </p>
            </div>
            <div className="modal-footer">
      ...
      {showVerificationErrorModal &&
        (() => {
          const account = accounts.find(
            (a) => a.id === showVerificationErrorModal,
          );
          if (!account) return null;
          const vReason =
            account.disabled_reason || verificationStatusMap[account.id];
              ...
              >
                ...
                <div className="modal-body">
                  <div className="error-detail">
                    ...
                    {vDetail?.lastMessage && (
                      <div
                        className="error-detail-message"
                        style={{ marginTop: 12 }}
                      >
                        {vDetail.lastMessage}
                      </div>
                    ...
                    {!isTos && vDetail?.validationUrl && (
                      <>
                        ...
                        >
                          <Copy size={14} />
                          {t(
                            "wakeup.errorUi.copyValidationUrl",
                            "复制验证地址",
                          )}
                        </button>
                      </>
      ...
      <TagEditModal
        isOpen={!!showTagModal}
        initialTags={
          accounts.find((acc) => acc.id === showTagModal)?.tags || []
        }
        initialNotes={
          accounts.find((acc) => acc.id === showTagModal)?.notes ?? ""
        }
        availableTags={availableTags}
